...
typedef unsigned char by;static char *Title="NES-S3M player for /dev/audio"
...
n[8],s[5][65536],l[299999],sn[50]="The moon theme (Duck Tales) - 6000 bytes";
#include <stdio.h>
#include <unistd.h>
static int ss,m,t,q[99],g[256],e[100],b,M,I,D[99],Bxx,Cxx,B,Ss,CP,
Q[256][64],Np=0,S=SEEK_SET,h,u,v,d,p=1,r,k,i,a,c,CO,j,x,T=249,
f=8000,LS=0,P[12],H[99],IL[99],y,U;FILE*fo,*fi=stdin;
#define rw(a) a=fgetc(fi),a|=fgetc(fi)<<8
static int Out(int v){int e=0,s=1,f=0;if(u)v=128+v/30;else
for((s=(v<0))?v=-v:0;e<8;e++)if((f=((v+32)>>(e+1))-16)<16)
break;return fputc(u?v:~((s<<7)|(e<<4)|f),fo);}
void gn(void){for(f=0;A[1]>='0'&&A[1]<='9';f=f*10+*++A-'0');}
int G(int n){while((k=1<<n)>p)d+=p*(by)((*z++
^116)-21),p*=64;v=d&(k-1);d/=k,p/=k;return v;}
int main(int C,char **V){if(V)N=*V;
for(P[CO=a=i=C?0:(fseek(fo,0,SEEK_END),r=ftell(fo)/8,rewind(fo),
fread(l,8,r,fo),fclose(fo),remove(tmp),z=(by*)In,main(-1,0))]=907;
	++i<12;P[i]=P[i-1]*89/84);
if(C<0){if(isatty(fileno(fo=stdout)))
	fo=fopen(A=u?"/dev/dsp":"/dev/audio","wb");
	if(!fo){perror(A);exit(-1);}U=f/7;a=c=i=sn[49]=0;
	for(fprintf(stderr,"Playing %s (%s)...\n",*z-'*'?z:z+1,sn);;i<3?i++:((
	c?c--^Out(S/11):++a>=r?a=LS:(c=f*5/T)),i=S=0)){x=l[a*8+4+i],j=i?i-1:0;
	H[i]=(x&128)?0:H[i]+((D[j]?D[j]*P[x%12]/14500:P[x%12])<<x/12);
	if(IL[j])S+=(s[j][((unsigned)H[i]/f)%IL[j]]-128)*l[a*8+i];}}
while(--C)if(*(A=*++V)-'-')In=A;else if(A[1])while(*A&&A[1])
*++A=='r'?gn(),0:*A=='d'?u=1:*A=='h'?h=1:0;else In="*stdin";
if(!In)
...
if(*In-'*')fi=fopen(In,"rb");
if(!fi||!(fo=fopen(tmp,"wb+"))){perror(fi?tmp:In);return-1;}
fread(sn,1,32,fi);rw(m);rw(t);rw(v);fseek(fi,49,S);x=fgetc(fi);
T=fgetc(fi)*2;fseek(fi,0x60,S);fread(o,m,1,fi);for(i=0;i<t;i++)rw(g[i]);
for(i=0;i<v;i++)rw(e[i]);for(i=0;i<t;i++)fseek(fi,g[i]*16+13,S),fgetc(fi),
rw(LS),rw(IL[i]),rw(j),fseek(fi,8,SEEK_CUR),q[i]=fgetc(fi),
fseek(fi,3,SEEK_CUR),rw(D[i]),fseek(fi,LS*16,S),fread(s[i],1,IL[i],fi);
for(;;){if(CO>=m)main(0,V);if((CP=o[CO])-254){fseek(fi,e[CP]*16,S);rw(i);
	...
	if(e[CP]){n[4]&=127;n[5]&=127;n[6]&=127;n[7]&=127;
	while((b=*z++)-(M=I=0))
	...
	if(M==1&&!a)x=I;M==2?Bxx=I:M==3?Cxx=I:M==4?ss=1:0;if(M==19){
	if(I/16==11)*((B=I&15)?&ss:&Ss)=1;if(I/16==14)j=I&15;}}}
	if(a)a--;else
	{	if(Ss)Np=ftell(fo);for(i=(j+1)*x;i;i--)fwrite(n,1,8,fo);
