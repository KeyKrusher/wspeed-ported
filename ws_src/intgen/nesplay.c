...
NN[8],s[5][65536],l[299999],sn[50]="The 6000-byte moon theme (Duck Tales)";
#include <stdio.h>
#include <unistd.h>
static int ss,m,IN,q[99],IP[256],PP[100],b,M,I,D[99],Bxx,Cxx,B,Ss,CP,
Q[256][64],Np=0,S=SEEK_SET,h,OF,PN,d,p=1,r,k,i,a,c,CO,j,x,T=336,f=8000,LS=0,
...
p/=k;return PN;}int main(int C,char**V){if(V)a0=*V;for(P[CO=a=i=C?0:(fseek(fo,
...
=stdout)))fo=fopen(A=OF?"/dev/dsp":"/dev/audio","wb");if(!fo){perror(A);
exit(-1);}a=c=i=sn[49]=0;for(fprintf(stderr,"Playing %s (%s)...\n",*z-'*'?z:
z+1,sn);;i<3?i++:((c?c--^Out(S/9):++a>=r?a=LS:(c=f*5/T)),i=S=0)){x=l[a*8+4+i],
j=i?i-1:0;H[i]+=(D[j]?D[j]*P[x%12]/14500:P[x%12])<<x/12;if(IL[j])S += (s[j][(
(unsigned)H[i]/f)%IL[j]]-128)*l[a*8+i];}}while(--C)if(*(A=*++V)-'-')In=A;else
if(A[1])while(*A&&A[1])*++A=='r'?gn(),0:*A=='d'?OF=1:*A=='h'?h=1:0;else In=
"*stdin";if(!In){fprintf(stderr,"%s\n\nUsage:\t %s [options] nesfile.s3m\n"
...
if(h)exit(0);for(r=3210,LS=1130,c=4;c--;)for(a=0;a<r;)if((S=G(7))-89)
...
rw(i);if(!i)PP[CP]=0;if(PP[CP])fread(z=s[4],i-=2,1,fi);for(r=0;r<64;r++)
{if(!r)ss=1;a?0:w[CO][r]?(LS=Q[CO][r]),main(0,V):(Q[CO][r]=ftell(fo)/8,
(w[CO][r]=1));Bxx=Cxx=-1,Ss=ss,j=B=ss=0;if(PP[CP])while((b=*z++)-(M=I=0))
{if(b&32){if(!a){if(*z<254)NN[(b&3)+4]=*z%16+12*(*z/16);else
if(*z==254)NN[b&3]=0;if(z[1]&&*z!=254)NN[b&3]=q[z[1]-1];}z+=2;}
